...
6. **自選股＋首頁** — 自選股 CRUD（依 §2 掛在 user 下）；首頁顯示自選股盤後行情與當日策略命中摘要。
7. **Telegram 推播** — 每日訊號＋pipeline 故障告警。
...
- **個股頁圖表上的指標顯示**：**跟隨當前顯示的價格序列**計算（原始價模式就用原始價算、還原價模式就用還原價算）。理由：MA 等主圖疊加指標若固定用還原價算，疊在原始價 K 棒上會因除權息缺口整段浮空；跟隨顯示價格與一般看盤軟體行為一致。
- **篩選條件、策略掃描、回測**：一律**還原價**（避免除息缺口造成假訊號；台股六到九月除息旺季不還原會假訊號淹沒）。
